...
The atomic extension provides instructions to atomically read, modify, and write back to memory between multiple harts running in the same memory space, including general-purpose load and stores, and fetch-and-op instructions that atomically performs specific computations on memory contents fetched. Unlike other loads/stores instructions, the atomic instructions are encoded in R-type instruction format, using absolute addressing rather than offsets to a base. This creates an essential divergence from HFI's nature of relative addressing. Therefore, while bounding the memory access, HFI cannot simply replace the base address and only check the offset with a single comparator. Instead, the hardware must first calculate the offset of the target address from the base address with a full subtractor. The offset can then be bounded as per usual. Implementations should account for this divergence and handle the extra cost of the subtractor hardware.
