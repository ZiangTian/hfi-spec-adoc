...
The atomic extension provides instructions to atomically read, modify, and write back to memory between multiple harts running in the same memory space, including general-purpose load and stores, and fetch-and-op instructions that atomically performs specific computations on memory contents fetched. Unlike other loads/stores instructions, the atomic instructions are encoded in R-type instruction format, using absolute addressing rather than offsets to a base. Therefore, while bounding the memory access, HFI cannot simply replace the base address and only check the offset with a single comparator. Instead, h-prefixed atomic instructions need to have the address in `rs1` checked specifically both against the base address and against the bound. This requires an additional 32-bit comparator.
