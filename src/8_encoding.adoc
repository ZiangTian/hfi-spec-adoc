...
H-prefixed base loads and stores automatically check against the bounds specified by the explicit region registers before serviced to memory. The base address stored in `rs1` is always ignored, and replaced with the base address of the explicit region.
...
The atomic extension provides instructions to atomically read, modify, and write back to memory between multiple harts running in the same memory space, including general-purpose load and stores, and fetch-and-op instructions that atomically performs specific computations on memory contents fetched. Unlike other loads/stores instructions, the atomic instructions are encoded in R-type instruction format, using a simple, absolute addressing modes without a base address. This diverges from HFI's nature of relative addressing. Therefore, while bounding the memory access, HFI cannot simply replace the base address and only check the offset with a single comparator. Instead, the hardware must first calculate the offset of the target address from the base address with a full subtractor. The offset can then be bounded as per usual. Implementations should account for this divergence and supply another full adder. Alternatively, it is also possible to reuse existing hardware (e.g. the effective address computation adders) to fulfill this full subtraction, at the cost of potentially adding a pipeline stage.
