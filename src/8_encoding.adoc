...
The non-h-prefixed HFI instructions are functional instructions interacting with specific internal registers. Corresponding to the new internal registers, these instructions primarily fulfill three tasks: HFI mode transition, exit handler configuration, and region manipulation. All of the non-h-prefixed instructions are encoded in a R-type format under the same custom opcode `HFUNCTION` to achieve maximum flexibility (See <<fig_r_type,Figure 5>>). The funct 3 field differentiates the instructions based on the tasks, and the funct 7 field further distinguishes the specific instruction. The specific encoding is shown in <<tab_enc_non_h,Table 5>>. The behavior of these instructions has mostly been covered in <<sec_sbx_setup,Chapter 3>>, <<sec_exit_handler,Chapter 4>> and <<sec_regions,Chapter 5>>, and this section mainly details their encoding.
